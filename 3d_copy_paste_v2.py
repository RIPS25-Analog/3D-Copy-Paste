# 🐱 CRITICAL FIX EDITION - Bulletproof Z-Coordinates + Smart Surface Selection! 🐱
import bpy
...
def select_placement_surface(scene_id, root_path, surface_selection_mode='random'):
    """
    IMPROVED: Select a surface for object placement with preference for appropriate surfaces
    """
    ...
    if os.path.exists(enhanced_plane_path):
        ...
        available_surfaces = []
        table_surfaces = []  # Prioritize tables/counters for tool placement
        
        # Add all other surfaces (PRIORITIZE THESE)
        for surface_type in ['table', 'desk', 'counter', 'surface']:
            if surface_type in plane_data.get('surfaces', {}):
                for surface in plane_data['surfaces'][surface_type]:
                    ...
                    table_surfaces.append(surface_info)
                    available_surfaces.append(surface_info)
        
        # Add floor if available (LOWER PRIORITY)
        if 'floor_id' in plane_data and plane_data['floor_id'] in plane_data['horizontal_planes']:
            ...
            available_surfaces.append(floor_surface)
        
        if available_surfaces:
            # 🚨 IMPROVED SURFACE SELECTION LOGIC
            if surface_selection_mode == 'random':
                # 70% chance to use table/counter, 30% chance for floor
                if table_surfaces and random.random() < 0.7:
                    selected = random.choice(table_surfaces)
                    print(f"🎯 Selected {selected['type']} (prioritized over floor)")
                    return selected
                else:
                    return random.choice(available_surfaces)
            elif surface_selection_mode == 'weighted':
                # Weight tables/counters higher than floors
                weights = []
                for surf in available_surfaces:
                    base_weight = surf['stats']['area']
                    if surf['type'] in ['table', 'desk', 'counter', 'surface']:
                        base_weight *= 3  # 3x more likely to select tables
                    weights.append(base_weight)
                total_weight = sum(weights)
                normalized_weights = [w / total_weight for w in weights]
                return random.choices(available_surfaces, weights=normalized_weights)[0]
                ...
                if table_surfaces:
                    return random.choice(table_surfaces)
...
def ensure_90_percent_visibility(inobject, bbox, surface_info, cam, scene, min_visibility=0.9):
    """
    🐱 ENHANCED VISIBILITY PROTECTION: Bulletproof system to ensure objects are visible
    """
    print(f"🐱 VISIBILITY CHECK: Ensuring {min_visibility:.0%} of object is visible...")
    
    bpy.context.view_layer.update()
    
    # 🚨 STEP 1: EMERGENCY UNDERGROUND CHECK
    ...
    # If object is completely underground, emergency lift
    if obj_top_z < 0:
        emergency_lift = 0.5 - obj_bottom_z  # Lift to 50cm above ground
        print(f"🚨 EMERGENCY: Object completely underground! Lifting by {emergency_lift:.3f}m")
        inobject.location.z += emergency_lift
        bbox.location.z += emergency_lift
        bpy.context.view_layer.update()
    
    # 🚨 STEP 2: SURFACE RELATIONSHIP CHECK
    surface_height = get_proper_surface_height(surface_info)
    obj_bottom_z = get_object_bottom_world_z(inobject)  # Re-check after potential lift
    
    # Ensure object is appropriately positioned relative to surface
    if obj_bottom_z < surface_height - 0.1:  # More than 10cm below surface
        ...
        if underground_ratio > 0.1:  # More than 10% underground
            ...
            # Lift object so bottom is on surface
            target_bottom_z = surface_height + 0.02  # 2cm above surface
    ...
    # 🚨 STEP 3: CAMERA VISIBILITY CHECK WITH ULTRA-DETAILED SAMPLING
    visibility_ratio = calculate_ultra_detailed_visibility(inobject, cam, scene)
    
    print(f"   Object visibility: {visibility_ratio:.1%}")
    
    if visibility_ratio < min_visibility:
        print(f"⚠️  Object only {visibility_ratio:.1%} visible! Finding better position...")
        
        # Try aggressive repositioning
        best_position = find_ultra_visible_position(inobject, bbox, surface_info, cam, scene, min_visibility)
        ...
        if best_position:
            ...
            # Re-check visibility
            new_visibility = calculate_ultra_detailed_visibility(inobject, cam, scene)
            ...
            if new_visibility >= min_visibility:
                ...
                return new_visibility >= 0.6  # Accept if at least 60%
        else:
            print(f"🐱 ⚠️  Could not find better position, keeping current placement")
            return visibility_ratio >= 0.6  # Accept if at least 60%
    else:
        print(f"🐱 ✅ Object visibility is excellent!")
        return True

def calculate_ultra_detailed_visibility(obj, cam, scene):
    """
    Ultra-detailed visibility calculation with many more sample points
    """
    from bpy_extras.object_utils import world_to_camera_view
    ...
    # Create a comprehensive grid of sample points
    sample_points = []
    ...
    # Add edge midpoints for even more detailed sampling
    edges = [
        # Bottom edges
        (min_corner, Vector([max_corner.x, min_corner.y, min_corner.z])),
        (Vector([max_corner.x, min_corner.y, min_corner.z]), Vector([max_corner.x, max_corner.y, min_corner.z])),
        (Vector([max_corner.x, max_corner.y, min_corner.z]), Vector([min_corner.x, max_corner.y, min_corner.z])),
        (Vector([min_corner.x, max_corner.y, min_corner.z]), min_corner),
        # Top edges  
        (Vector([min_corner.x, min_corner.y, max_corner.z]), Vector([max_corner.x, min_corner.y, max_corner.z])),
        (Vector([max_corner.x, min_corner.y, max_corner.z]), max_corner),
        (max_corner, Vector([min_corner.x, max_corner.y, max_corner.z])),
        (Vector([min_corner.x, max_corner.y, max_corner.z]), Vector([min_corner.x, min_corner.y, max_corner.z])),
    ]
    
    for edge_start, edge_end in edges:
        edge_midpoint = (edge_start + edge_end) / 2
        sample_points.append(edge_midpoint)
    ...
    for point in sample_points:
        ...
        # Check if point is in camera view with some margin
        if (0.01 <= co_2d.x <= 0.99 and 0.01 <= co_2d.y <= 0.99 and co_2d.z > 0):
            visible_count += 1
    
    return visible_count / total_count if total_count > 0 else 0

def find_ultra_visible_position(inobject, bbox, surface_info, cam, scene, min_visibility, max_attempts=30):
    """
    More aggressive position finding with wider search area
    """
    ...
    # Try positions across the entire surface
    for attempt in range(max_attempts):
        # Use a more systematic grid search
        grid_size = int(sqrt(max_attempts))
        row = attempt // grid_size
        col = attempt % grid_size
        
        # Map to surface coordinates
        x_ratio = col / (grid_size - 1) if grid_size > 1 else 0.5
        y_ratio = row / (grid_size - 1) if grid_size > 1 else 0.5
        
        # Add some randomness
        x_ratio += random.uniform(-0.1, 0.1)
        y_ratio += random.uniform(-0.1, 0.1)
        x_ratio = np.clip(x_ratio, 0.1, 0.9)
        y_ratio = np.clip(y_ratio, 0.1, 0.9)
        
        test_x = surface_info['stats']['min'][0] + x_ratio * surface_width
        test_y = surface_info['stats']['min'][1] + y_ratio * surface_length
        
        # Calculate proper Z position for this surface
        surface_height = get_proper_surface_height(surface_info)
        obj_height = get_object_top_world_z(inobject) - get_object_bottom_world_z(inobject)
        test_z = surface_height + obj_height * 0.15  # 15% above surface
        
        # Ensure Z is never negative
        test_z = max(test_z, 0.1)
        ...
        visibility = calculate_ultra_detailed_visibility(inobject, cam, scene)
        ...
        if visibility > best_visibility:
            ...
            if visibility >= min_visibility:
                print(f"   Found excellent position at attempt {attempt + 1}: {visibility:.1%} visible")
                break
    ...
    print(f"   Best position found: {best_visibility:.1%} visibility")
    
    if best_position and best_visibility > calculate_ultra_detailed_visibility(inobject, cam, scene):
...
def place_object_on_surface_FIXED(inobject, bbox, surface_info, best_parameter, final_scale):
    """
    COMPLETELY FIXED: Properly place object on surface with bulletproof Z calculation
    """
    ...
    # Get surface height with EXTRA SAFETY
    surface_height = get_proper_surface_height(surface_info)
    
    # 🚨 CRITICAL FIX: Ensure surface height is never negative or unreasonable
    if surface_height < -2.0 or surface_height > 5.0:
        print(f"⚠️  WARNING: Suspicious surface height {surface_height:.3f}, using safe default")
        surface_height = 0.0  # Use ground level as safe fallback
    
    # Position object
    inobject.location.x = best_parameter['sample_position_X']
    inobject.location.y = best_parameter['sample_position_Y']
    
    # 🚨 BULLETPROOF Z CALCULATION
    # Method 1: Simple and safe - place object bottom on surface
    scaled_bottom_offset = local_min_z * final_scale
    initial_z = surface_height - scaled_bottom_offset + 0.02  # 2cm clearance
    
    # 🚨 CRITICAL SAFETY CHECK: Never allow negative Z positions
    if initial_z < 0:
        print(f"⚠️  Calculated Z would be negative ({initial_z:.3f}), forcing positive")
        initial_z = 0.05  # 5cm above ground level minimum
    
    inobject.location.z = initial_z
    ...
    bpy.context.view_layer.update()
    actual_bottom_z = get_object_bottom_world_z(inobject)
    
    if actual_bottom_z < 0:
        print(f"🚨 EMERGENCY FIX: Object bottom at {actual_bottom_z:.3f}, lifting to safe position")
        emergency_lift = 0.1 - actual_bottom_z  # Lift to 10cm above ground
        inobject.location.z += emergency_lift
        bbox.location.z += emergency_lift
        bpy.context.view_layer.update()
        actual_bottom_z = get_object_bottom_world_z(inobject)
    ...
    print(f"  Initial Z: {initial_z:.3f}")
    print(f"  Final object Z: {inobject.location.z:.3f}")
    print(f"  Actual bottom Z: {actual_bottom_z:.3f}")
    
    return actual_bottom_z
...
def main(args):
    ...
    out_folder_name = 'insertion_ilog{}_istren{}_{}_surf{}_cov{}-{}_CRITICAL_FIX'.format(
    ...
    for iter in range(max_iter):
        for index, scene_id in enumerate(train_data_ids):
            ...
            try:
                ...
                insert_log_dict['critical_fixes_applied'] = True
                ...
                if bbox_coords:
                    ...
                    if yolo_bbox:
                        ...
                        positive_z = bbox.location[2] > 0
                        
                        # Final visibility check
                        final_visibility = calculate_ultra_detailed_visibility(inobject, cam, scene)
                        
                        if meets_minimum and within_maximum and positive_z and final_visibility >= 0.9:
                            print(f"🐱 ✅ CRITICAL FIXES SUCCESSFUL: All problems solved! {final_visibility:.1%} visible!")
                        elif positive_z and meets_minimum and within_maximum:
                            print(f"🐱 ✅ FIXED MAIN ISSUES: Good positioning and size, {final_visibility:.1%} visible!")
                        elif not positive_z:
                            print(f"🐱 ⚠️  Z-coordinate still problematic: {bbox.location[2]:.3f}")
                        else:
                            print(f"🐱 ⚠️  Some issues remain, but major problems fixed")
                        
                        insert_log_dict['final_visibility_ratio'] = final_visibility
                        insert_log_dict['final_z_coordinate'] = float(bbox.location[2])
                ...
                insert_3D_info = {
                    ...
                    'critical_fixes_enforced': True,
                    'bulletproof_z_coordinates': True,
                    'smart_surface_selection': True,
                    'ultra_detailed_visibility': True,
                    'emergency_underground_protection': True,
                ...
                print(f"🐱 🔧 CRITICAL FIX: Successfully saved composite image for scene {scene_id} ({processing_time:.2f}s) | Avg: {current_avg:.2f}s")
                
            except Exception as e:
                print(f"❌ Error processing scene {scene_id}: {str(e)}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='🐱 CRITICAL FIX EDITION - Bulletproof Z-Coordinates + Smart Surface Selection! 🐱')
